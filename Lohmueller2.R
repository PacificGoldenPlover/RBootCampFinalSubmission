...
setwd("C:/Users/pacif/Documents/Lohmueller Data")
...
length(pvals)
exp_pvals<-(1:length(pvals))/length(pvals)
sort_pvals<-sort(pvals, decreasing = FALSE)
log_sort_pvals<- -log10(sort_pvals)
log_exp_pvals<- -log10(exp_pvals)
plot(log_exp_pvals, log_sort_pvals, xlab = "expected", ylab = "observed")
######################################################################
phenotype<-z$glucose_mmolperL
quantile(phenotype) 
d<-density(phenotype)
plot(d)
abline(v=quantile(phenotype,0.75),col=2)
abline(v=quantile(phenotype,0.25),col=4)
